...
Status: `complete`
Notes: `backend/study-guide-agent/app/prompt_lab/sample_inputs/` now holds the initial curated prompt-lab sample set as checked-in `PromptLabSampleInput` JSON files, `backend/study-guide-agent/app/prompt_lab/samples.py` now resolves those samples by stable id, and the backend plus frontend now expose read-only prompt-lab sample routes so the later reviewer UI can preload a curated case or fetch the full catalog without mixing this data into teacher-facing defaults. Focused validation passed with `cd backend/study-guide-agent && uv run pytest tests/unit/test_prompt_lab_samples.py -q` and `cd frontend && npm run typecheck`, and the repo done gate `./scripts/validate-task.sh` also passed.

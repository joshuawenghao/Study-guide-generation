...
- The shared section parser now also strips malformed inline quoted-list annotations from model-returned JSON strings before a second parse attempt, and the assessment-passage prompt now explicitly forbids those bracketed clue-list annotations inside passage paragraphs, closing the live UI parse failure that had been observed in `assessment_passage`

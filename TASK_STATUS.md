...
Status: `complete`
Notes: The IFC must-have checklist has now been verified against passing local validations, the shipped control paths, and a fresh live full-workflow demo. `./scripts/validate-task.sh` passed, the current repo gate now covers 57 passing backend unit and integration tests plus the built-frontend smoke path, the focused IFC slice `uv run pytest tests/integration/test_agent.py tests/unit/test_renderer.py tests/unit/test_validator_node.py -q` passed with 8 tests, and `uv run python run_demo.py --mode full-workflow` now succeeds again from `backend/study-guide-agent/`, writing `demo-output/study-guide-full-demo.pdf` with 18 preview sections, 0 failed sections, 0 best-effort sections, and 3 non-blocking reading-level warnings. The shipped backend now also strips HTML-like markup from model text before preview and PDF rendering, auto-repairs assessment-answer quote embedding when Gemini only fills `evidence_quote`, and realigns answer-key assessment answers back to the upstream assessment-question text and exact passage-quote candidates on both first-pass generation and answer-key retries. A fresh browser rerun of the Grade 12 nursing scenario on a clean local stack (`3002 -> 8001`) kept the markup fix intact and narrowed the answer-key quote mismatches, but it still ended with 1 failed section and 1 best-effort section in `answer_key`, so answer-key quote fidelity remains model-variable for that higher-grade healthcare content. The signoff covers dependency ordering and one-pass retry in `backend/study-guide-agent/tests/integration/test_agent.py`, canonical preview/render ordering in `backend/study-guide-agent/app/nodes/renderer.py`, answer-key page breaks in `backend/study-guide-agent/app/templates/study_guide.html.j2`, market- and grade-aware prompting in `backend/study-guide-agent/app/prompts/system_prompt.py`, hard-validator aggregation in `backend/study-guide-agent/app/nodes/validator.py`, and frontend isolation through the thin backend proxy in `frontend/app/api/generate/route.ts`. The local runtime now auto-loads `backend/study-guide-agent/.env` for backend runs, so the demo and API-backed workflow no longer depend on separately exported shell variables just to find `GOOGLE_API_KEY`.

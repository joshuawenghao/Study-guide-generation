...
Status: `partial`
Notes: The repo still has the reusable frontend-side contract needed for deployment: the Next.js proxy routes remain thin and environment-driven around `ADK_BACKEND_URL`, the local parity path still exercises the production frontend against the Cloud Run-style backend container, and the deployment docs now describe Firebase App Hosting as the canonical managed frontend target. However, the old Vercel-specific completion no longer satisfies the current spec: the repo does not yet include a validated Firebase App Hosting staging configuration, a recorded staging App Hosting domain, or a completed remote staging proof that the deployed frontend can talk to the staging Cloud Run backend without code edits.

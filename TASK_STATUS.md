...
- The backend now also keeps `assessment_questions.answer_expectation` and `assessment_questions.evidence_requirement` as grounded teacher-guidance fields rather than final answers or verbatim quotes, asks Gemini for explicit `assessment_answers` entries in the answer key, hard-validates assessment-answer `evidence_quote` directly against the assessment passage, and defensively re-normalizes `answer_key` during validation and rendering so raw model assessment answers cannot silently bypass the quote guard

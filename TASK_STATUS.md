...
- The frontend deployment slice now includes checked-in Firebase App Hosting config files at `frontend/apphosting.yaml` and `frontend/apphosting.staging.yaml`; focused validation passed with `cd frontend && npm run build`, and the repo done gate `./scripts/validate-task.sh` now also passes on that state, but the remote staging rollout is still pending because no App Hosting backend/domain has been created yet and the local workspace currently does not have the Firebase CLI installed
...
Status: `partial`
Notes: The repo now includes a checked-in Firebase App Hosting configuration under `frontend/apphosting.yaml` plus a staging-specific override in `frontend/apphosting.staging.yaml`, while the Next.js proxy routes remain thin and environment-driven around `ADK_BACKEND_URL`, and the deployment docs now describe the exact staging App Hosting backend creation flow for `manabie-ai`, `asia-northeast1`, backend name `study-guide-frontend-staging`, environment name `staging`, and app root directory `frontend`. Focused validation passed with `cd frontend && npm run build`, and the repo done gate `./scripts/validate-task.sh` also passed. On 2026-05-29 the external rollout advanced materially: the `cloud-run-source-deploy` Artifact Registry repository was created successfully, the backend image was rebuilt to include the missing `study_guide_agent` loader package, an explicit `linux/amd64` image was built and pushed to `asia-northeast1-docker.pkg.dev/manabie-ai/cloud-run-source-deploy/study-guide-agent-staging:manual-20260529-amd64fix1`, and the staging Cloud Run backend is now live at `https://study-guide-agent-staging-angcxvrtzq-an.a.run.app`. The staging frontend config now points `ADK_BACKEND_URL` at that real backend URL. The latest retry also moved the App Hosting path much further: the active user now has `roles/developerconnect.admin`, the App Hosting flow generated the Developer Connect service identity, granted the required Secret Manager role to that service agent, created and reused the GitHub connection `apphosting-github-conn-uo8k2vs`, and linked the repository `joshuawenghao/Study-guide-generation` on branch `main`. The remaining blocker is now narrowed to the Firebase web-app control plane itself: after the repo link succeeds, `firebase apphosting:backends:create ...` fails when Firebase CLI calls `GET https://firebase.googleapis.com/v1beta1/projects/manabie-ai/webApps` and receives `404 Firebase project 493716394242 not found`, and standalone `firebase apps:list WEB --project manabie-ai` and `firebase projects:addfirebase manabie-ai` continue to fail on that same missing/permission path. The repo therefore still lacks a recorded App Hosting staging domain and the final remote proof that the deployed frontend can talk to the staging Cloud Run backend.

### Task 13.6 — Run staged deployment checkpoints

Status: `not started`
Notes: No remote deployment checkpoints have been recorded yet after Phases 7, 10, or 12. The backend half of the first non-production rollout is now live, but frontend checkpoint execution is still blocked because App Hosting backend creation now gets through Developer Connect and GitHub repo linking before failing on the Firebase web-app API, which still does not recognize `manabie-ai` as a fully available Firebase project for the current caller.

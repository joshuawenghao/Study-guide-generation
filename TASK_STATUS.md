...
- The frontend preview renderer now covers every shipped section type with an explicit layout path, so current preview sections no longer rely on the generic JSON-stringify fallback; the learning-target competency summary is now stacked vertically at full width, and verbose structured sections such as strategy lists, check-in questions, assessment questions, step-up prompts, warmups, intros, core explainers, and key points now render in top-to-bottom cards that read in a clearer order on the page

...
Status: `partial`
Notes: The repo now includes a checked-in Firebase App Hosting configuration under `frontend/apphosting.yaml` plus a staging-specific override in `frontend/apphosting.staging.yaml`, while the Next.js proxy routes remain thin and environment-driven around `ADK_BACKEND_URL`, and the deployment docs now describe the exact staging App Hosting backend creation flow for `manabie-ai`, `asia-northeast1`, backend name `study-guide-frontend-staging`, environment name `staging`, and app root directory `frontend`. Focused validation passed with `cd frontend && npm run build`, and the repo done gate `./scripts/validate-task.sh` also passed. This task remains partial because the external staging rollout has not yet been executed: the repo still lacks a recorded App Hosting staging domain, a replaced staging `ADK_BACKEND_URL` value, and a completed remote proof that the deployed frontend can talk to the staging Cloud Run backend without code edits. The local workspace also does not currently have the Firebase CLI installed, so no App Hosting backend creation or rollout command was run in this pass.

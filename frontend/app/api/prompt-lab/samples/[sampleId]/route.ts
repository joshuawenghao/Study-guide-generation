...
export async function GET(
  _request: Request,
  context: { params: { sampleId: string } },
): Promise<Response> {
  let upstreamResponse: Response;

  try {
    upstreamResponse = await fetch(
      `${getBackendUrl()}/prompt-lab/samples/${encodeURIComponent(context.params.sampleId)}`,
      {
        method: "GET",
        headers: {
    ...
    return Response.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "The backend prompt-lab sample endpoint could not be reached.",
      },
      { status: 502 },
    );
  }
    ...
    return Response.json(
      {
        error:
          errorBody ||
          "The backend prompt-lab sample endpoint returned an error.",
      },

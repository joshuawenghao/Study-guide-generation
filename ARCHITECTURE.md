...
flowchart TD
    ...
    subgraph RetryLoop ["Retry loop — while failed_sections:"]
      RL["ctx.run_node() for each\nfailed section only\n(failure-specific retry prompt\n+ TEMP_RETRY)\nskipping downstream sections\nalready regenerated by the same retry pass"]
        RVAL["validator_node\n(second pass)"]
    end
...
| 6    | retry loop (failed sections only)                                                                      | `while` loop + `ctx.run_node()` — one retry per failed section, skipping downstream sections already refreshed by the same upstream retry |
...
Blueprint
  ...
  deep_dive_dimensions: string[] # 2-4 subject-appropriate compare/contrast labels; generated by blueprint node
  sub_competencies: SubCompetency[]
  core_concept: string
```
...
`ProgressTracker.tsx` consumes the SSE stream from the API proxy and renders a vertical step list. Each `node_complete` event checks off the corresponding step. During a retry pass, downstream regeneration events such as `check_in`, `assessment_questions`, `step_up`, or `answer_key` keep the tracker in the retry state until validation restarts, so the UI does not flicker back to plain generation mid-retry. The tracker also shows the current active node and elapsed time.

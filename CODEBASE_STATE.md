...
- The scaffold-native ADK CLI and eval path now also works end to end from `backend/study-guide-agent/app/`, using a dedicated conversational adapter for `agents-cli run` and `agents-cli eval run` without replacing the real typed study-guide workflow used by `/generate`.
...
- `backend/study-guide-agent/app/__init__.py` now exports a scaffold-eval-friendly `app` surface backed by `app/eval_agent.py`, so ADK's `./app` loader can run conversational CLI and eval flows while the shipped `POST /generate` path continues to use the real workflow in `app/agent.py`.
